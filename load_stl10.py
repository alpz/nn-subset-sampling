...
#Load stl 10 data with custom indices
...
class STL10(VisionDataset):
    ...
    def __init__(
        ...
        if self.split == 'train':
            self.data, self.labels = self.__loadfile(
                self.train_list[0][0], labels_file_path)
            ...
            self.data, self.labels = self.__loadfile(
                ...
                self.test_list[0][0], labels_file_path)
    ...
    def __loadfile(self, data_file: str, labels_file_path: Optional[str] = None) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        labels = None
        if labels_file_path:
            ...
            with open(labels_file_path, 'rb') as f:
                ...
                #load saved labels
                labels = np.load(f)
                print("loaded labels ", np.shape(labels))
    ...
    def _check_integrity(self) -> bool:
        raise Exception("check integrity not implemented")
        root = self.root
